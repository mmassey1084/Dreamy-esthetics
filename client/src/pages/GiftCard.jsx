...
function validate(values) {
  const errors = {};
  if (!values.fromName || values.fromName.trim().length < 2)
    errors.fromName = "Your name is required.";
  if (!values.toName || values.toName.trim().length < 2)
    errors.toName = "Recipient name is required.";
  if (!values.toEmail || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(values.toEmail))
    errors.toEmail = "Valid recipient email is required.";
  const amt = Number(values.amount);
  if (!amt || amt < 10 || amt > 1000)
    errors.amount = "Amount must be between $10 and $1000.";
  if ((values.message || "").length > 240)
    errors.message = "Message must be 240 characters or less.";
  return errors;
}
...
export default function GiftCard() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setStatus({
        state: "success",
        message: "Gift card request sent! (Demo endpoint)",
      });
    } catch (err) {
      setStatus({
        state: "error",
        message: err.message || "Gift card failed.",
      });
    }
  };
      ...
      <div className="container">
        <div className="grid2">
          <div className="card" data-reveal>
            ...
            <p className="muted">
              A perfect treat for someone you love. This form sends to the Node
              server.
            </p>
            ...
            <form className="form" onSubmit={submit} noValidate>
              <div className="form__row">
                <label className="field">
                  ...
                  <input
                    className={`input ${
                      touched.fromName && errors.fromName ? "input--error" : ""
                    }`}
                    name="fromName"
                    value={values.fromName}
                    onChange={onChange}
                    onBlur={() => mark("fromName")}
                  />
                  {touched.fromName && errors.fromName && (
                    <span className="field__error">{errors.fromName}</span>
                  )}
                </label>
                <label className="field">
                  ...
                  <input
                    className={`input ${
                      touched.toName && errors.toName ? "input--error" : ""
                    }`}
                    name="toName"
                    value={values.toName}
                    onChange={onChange}
                    onBlur={() => mark("toName")}
                  />
                  {touched.toName && errors.toName && (
                    <span className="field__error">{errors.toName}</span>
                  )}
                </label>
              </div>
              ...
              <label className="field">
                ...
                <input
                  className={`input ${
                    touched.toEmail && errors.toEmail ? "input--error" : ""
                  }`}
                  name="toEmail"
                  type="email"
                ...
                {touched.toEmail && errors.toEmail && (
                  <span className="field__error">{errors.toEmail}</span>
                )}
              </label>
          ...
          <div className="card card--glow" data-reveal>
            ...
            <p className="muted">
              Want to buy specific services instead? Use the cart from the
              Booking page.
            </p>
