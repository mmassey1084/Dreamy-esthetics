...
export default function CartDrawer() {
  ...
  const checkout = async (e) => {
    ...
    try {
      ...
      setStatus({
        state: "success",
        message: `Success! Order #${res.orderId} created.`,
      });
      cart.clear();
    } catch (err) {
  ...
  return (
    <>
      <div
        className={`drawer__backdrop ${cart.isOpen ? "is-open" : ""}`}
        onClick={cart.close}
      />
      <aside
        className={`drawer ${cart.isOpen ? "is-open" : ""}`}
        ...
        <div className="drawer__header">
          <div>
            ...
            <div className="drawer__sub">
              {cart.count} item(s) • {formatMoney(cart.subtotal)}
            </div>
          </div>
          <button
            className="iconbtn"
          ...
          {cart.items.length === 0 ? (
            <div className="empty">
              ...
              <div className="empty__text">
                Add services from the Booking page.
              </div>
            </div>
          ) : (
                ...
                <li className="cartitem" key={item.key}>
                  <div className="cartitem__meta">
                    ...
                    <div className="cartitem__sub">
                      {item.group} • {formatMoney(item.price)}
                    </div>
                  </div>
          ...
          <form className="form" onSubmit={checkout} noValidate>
            ...
            <div className="notice">
              ...
              <div className="muted" style={{ marginTop: ".35rem" }}>
                Demo checkout (no payment gateway). The server receives the
                order and returns an order ID.
              </div>
            </div>
